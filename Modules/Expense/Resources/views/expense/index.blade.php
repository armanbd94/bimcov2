            ...
            <div class="card-header flex-wrap py-5">
                <form method="POST" id="form-filter" class="col-md-12 px-0">
                    <div class="row">
                        ...
                        <x-form.selectbox labelName="Approval Status" name="status" col="col-md-4" class="selectpicker">
                        @foreach (EXPENSE_APPROVE_STATUS as $key => $value)
...
@include('expense::expense.status-modal')
@endsection
...
<script>
    ...
    $(document).ready(function(){
        ...
        $(document).on('click','.change_status',function(){
            $('#approve_status_form #expense_id').val($(this).data('id'));
            $('#approve_status_form #expense_status').val($(this).data('status'));
            $('#approve_status_form #expense_status.selectpicker').selectpicker('refresh');
            $('#approve_status_modal').modal({
                keyboard: false,
                backdrop: 'static',
            });
            $('#approve_status_modal .modal-title').html('<span>Change Expense Status</span>');
            $('#approve_status_modal #status-btn').text('Change Status');
                
        });

        $(document).on('click','#status-btn',function(){
            var expense_id     = $('#approve_status_form #expense_id').val();
            var expense_status =  $('#approve_status_form #expense_status option:selected').val();
            if(expense_id && expense_status)
            {
                $.ajax({
                    url: "{{route('expense.change.status')}}",
                    type: "POST",
                    data: {expense_id:expense_id,expense_status:expense_status,_token:_token},
                    dataType: "JSON",
                    beforeSend: function(){
