...
class FinishedGoodsStockLedgerController extends BaseController
{
    ...
    {
        if(permission('finish-goods-ledger-access')){
            $this->setPageData('Finish Goods Ledger','Finish Goods Ledger','fas fa-file',[['name'=>'Finish Goods Ledger']]);
    ...
    protected function previous_data(int $id, $date)
    {
        ...
        $opening_price = 0;
        $opening_date = '';
        $opening_stock = DB::table('products')->where('id',$id)->first();
        if($opening_stock){
            $opening_stock_qty = $opening_stock->opening_stock_qty ? $opening_stock->opening_stock_qty : 0;
            $opening_price = $opening_stock->price ? $opening_stock->price : 0;
        ...
        $productionProducts = DB::table('productions as pro')
            ->selectRaw('pro.*,u.operator,u.operation_value,p.price')
            ->join('products as p', 'pro.product_id', '=', 'p.id')
            ->join('units as u', 'pro.unit_id', '=', 'u.id')
        ...
        $saleProducts = DB::table('sale_products as sp')
            ->selectRaw('sp.*,u.operator,u.operation_value,p.price')
        ...
        $saleReturnProduct = DB::table('sale_return_products as srp')
            ->selectRaw('srp.*,u.operator,u.operation_value,sr.return_no')
    ...
    protected function sold_data(int $id, $date) : array
    {
        ...
        $soldProducts = DB::table('sale_products as sp')
            ->selectRaw('sp.*,u.operator,u.operation_value,p.price,p.tax_method,s.invoice_no')
            ->join('products as p', 'sp.product_id', '=', 'p.id')
            ->join('sales as s', 'sp.sale_id', '=', 's.id')
            ->join('units as u', 'sp.sale_unit_id', '=', 'u.id')
            ->where('sp.product_id', $id)
    ...
    protected function production_data(int $id, $date)
    {
        ...
        if (!$saleReturnProduct->isEmpty()) {
            foreach ($saleReturnProduct as $product) {
                ...
                $total_returned_product_qty += $return_qty;
                $return_number_list[] = $product->return_no;
            }
        }
