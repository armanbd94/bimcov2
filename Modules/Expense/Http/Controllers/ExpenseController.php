            ...
            if(permission('expense-access')){
                ...
                foreach ($list as $value) {
                    ...
                    if(permission('expense-approve')){
                        if($value->status != 1){
                            $action .= ' <a class="dropdown-item change_status"  data-id="' . $value->id . '" data-name="' . $value->voucher_no . '" data-status="'.$value->status.'"><i class="fas fa-check-circle text-success mr-2"></i> Change Status</a>';
                        }
                    }
                    ...
                    $row[] = EXPENSE_APPROVE_STATUS_LABEL[$value->status];
            ...
            if(permission('expense-approve')){
                if ($request->expense_status) {
                    ...
                    try {
                        ...
                        {
                            $output = ['status'=>'success','message'=>'Purchase Status Changed Successfully'];
                        }else{
                            $output = ['status'=>'error','message'=>'Failed To Change Purchase Status'];
                        }
                        DB::commit();
                    } catch (\Throwable $th) {
                        DB::rollback();
