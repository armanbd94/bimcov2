                ...
                <form id="store_or_update_form" method="post">
                        ...
                        <div class="col-md-12">
                            <div class="card card-custom card-fit card-border">
                                <div class="card-body py-5">
                                    <div class="row">
                                        <x-form.selectbox labelName="Finish Goods" name="product_id" required="required"
                                            onchange="formulationData(this.value)" col="col-md-4" class="selectpicker">
                                            @if (!$products->isEmpty())
                                            @foreach ($products as $product)
                                            <option value="{{ $product->id }}"
                                                {{ $production->product_id == $product->id ? 'selected' : '' }}
                                                data-unitid="{{ $product->unit_id }}"
                                                data-unitname="{{ $product->unit->unit_name }}">
                                                {{ $product->name.' - '.$product->code }}</option>
                                            @endforeach
                                            @endif
                                        </x-form.selectbox>
                                        <x-form.selectbox labelName="FG Serial No" name="formulation_no"
                                            required="required"
                                            onchange="setFrmulationData();formulationMaterials(this.value);"
                                            col="col-md-4" class="selectpicker">
                                            @if (!$formulations->isEmpty())
                                            @foreach ($formulations as $item)
                                            <option value="{{ $item->id }}"
                                                class="{{ (($production->formulation_id == $item->id) && ($item->status == 1)) ? '' : 'bg-secondary' }}"
                                                {{ (($production->formulation_id == $item->id) && ($item->status == 1)) ? 'selected' : 'disabled' }}
                                                data-unitid="{{ $item->unit_id }}"
                                                data-unitname="{{ $item->unit->unit_name }}"
                                                data-totalfgqty="{{ $item->total_fg_qty }}">{{ $item->formulation_no }}
                                            </option>
                                            @endforeach
                                            @endif
                                        </x-form.selectbox>
                                        <div class="col-md-4">
                                            <div class="row" style="justify-content: space-evenly;padding-top:28px;">
                                                <button type="button" class="btn btn-sm btn-success col-md-5"
                                                    id="active-btn" onclick="showFormulationData('active')">Show Active
                                                    FG SL</button>
                                                <button type="button" class="btn btn-sm btn-secondary col-md-5"
                                                    id="all-btn" onclick="showFormulationData('all')">Show All FG
                                                    SL</button>
                                            </div>
                                        </div>
                                        <div class="form-group col-md-4 required">
                                            ...
                                            <input type="text" class="form-control" name="serial_no" id="serial_no"
                                                value=" {{ $production->formulation_id }}" readonly />
                                        </div>
                                        <div class="form-group col-md-4 required">
                                            <label for="date">Date</label>
                                            <input type="text" class="form-control date" name="date" id="date"
                                                value="{{ $production->date }}" readonly />
                                        </div>

                                        <div class="form-group col-md-4 required">
                                            <label for="unit_name">Unit</label>
                                            <input type="text" class="form-control" name="unit_name" id="unit_name"
                                                value="{{ $production->unit->unit_name }}" readonly />
                                            <input type="hidden" class="form-control" name="unit_id" id="unit_id"
                                                value="{{ $production->unit_id }}" readonly />
                                        </div>
                                        <div class="form-group col-md-4 required">
                                            <label for="total_fg_qty">Total FG Quantity</label>
                                            <input type="text" class="form-control" name="total_fg_qty"
                                                id="total_fg_qty" value=" {{ $production->total_fg_qty }}"
                                                onkeyup="totalCostCalculation()" />
                                        </div>
                                        <x-form.selectbox labelName="Material Warehouse" name="mwarehouse_id"
                                            required="required" col="col-md-4" class="selectpicker">
                                            @if (!$warehouses->isEmpty())
                                            @foreach ($warehouses as $warehouse)
                                            <option value="{{ $warehouse->id }}"
                                                {{ $production->mwarehouse_id == $warehouse->id ? 'selected' : '' }}>
                                ...
                                <tr>
                                    <td>
                                        <input type="text" class="form-control"
                                            value="{{ $item->material_code.' - '.$item->material_name }}"
                                            name="materials[{{ $key+1 }}][material_name]"
                                            id="materials_{{ $key+1 }}_material_name" data-id="{{ $key+1 }}" readonly>
                                        <input type="hidden" class="form-control"
                                            value="{{ $item->pivot->material_id }}"
                                            name="materials[{{ $key+1 }}][material_id]"
                                            id="materials_{{ $key+1 }}_material_id" data-id="{{ $key+1 }}" readonly>
                                    </td>
                                    <td>
                                        <input type="text" class="form-control text-center"
                                            value="{{ $item->category->name }}" name="materials[{{ $key+1 }}][category]"
                                            id="materials_{{ $key+1 }}_category" data-id="{{ $key+1 }}" readonly>
                                        <input type="hidden" class="form-control text-center prefix" value="+"
                                            name="materials[{{ $key+1 }}][prefix]" id="materials_{{ $key+1 }}_prefix"
                                            data-id="{{ $key+1 }}" readonly>
                                    </td>
                                    <td>
                                        <input type="hidden" class="form-control" value="{{ $item->unit_id }}"
                                            name="materials[{{ $key+1 }}][unit_id]" id="materials_{{ $key+1 }}_unit_id"
                                            data-id="{{ $key+1 }}">
                                        <input type="text" class="form-control text-center"
                                            value="{{ $item->unit->unit_name }}"
                                            name="materials[{{ $key+1 }}][unit_name]"
                                            id="materials_{{ $key+1 }}_unit_name" data-id="{{ $key+1 }}" readonly>
                                    </td>
                                    <td>
                                        <input type="text" class="form-control text-right"
                                            value="{{ $item->pivot->rate }}" name="materials[{{ $key+1 }}][rate]"
                                            id="materials_{{ $key+1 }}_rate" data-id="{{ $key+1 }}" readonly>
                                    </td>
                                    <td>
                                        <input type="text" class="form-control text-right qty"
                                            value="{{ $item->pivot->qty }}" name="materials[{{ $key+1 }}][qty]"
                                            id="materials_{{ $key+1 }}_qty" data-id="{{ $key+1 }}" readonly>
                                        <input type="hidden" class="form-control text-right per_unit_qty"
                                            name="materials[{{ $key+1 }}][per_unit_qty]"
                                            value="{{ ($item->pivot->qty / $production->total_fg_qty) }}"
                                            id="materials_{{ $key+1 }}_per_unit_qty" data-id="{{ $key+1 }}" >
                                    </td>
                                    <td>
                                        <input type="text" class="form-control text-right total"
                                            value="{{ $item->pivot->total }}" name="materials[{{ $key+1 }}][total]"
                                            id="materials_{{ $key+1 }}_total" data-id="{{ $key+1 }}" readonly>
                                        <input type="hidden" class="form-control text-right per_unit_total"
                                            name="materials[{{ $key+1 }}][per_unit_total]"
                                            value="{{ number_format(($item->pivot->total / $production->total_fg_qty),4,'.','') }}"
                                            id="materials_{{ $key+1 }}_per_unit_total" data-id="{{ $key+1 }}" readonly>
                    ...
                    <div class="col-md-12 py-5">
                        <div class="col-md-12 text-center">
                            <h5 class="bg-warning text-white p-3" style="width:250px;margin: 20px auto 10px auto;">
                                Materials Adjustment</h5>
                        </div>
                        <table class="table table-borderless pb-5" id="material_mixing_table">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    <td>
                                        <select class="form-control selectpicker"
                                            name="packaging_materials[{{ $key+1 }}][material_id]"
                                            id="packaging_materials_{{ $key+1 }}_material_id" data-id="{{ $key+1 }}"
                                            onchange="setRowValue('{{ $key+1 }}')" data-live-search="true">
                                            ...
                                            <option value="{{ $material->id }}"
                                                ...
                                                data-unitname="{{ $material->unit->unit_name }}"
                                                data-rate="{{ $material->cost }}">
                                    ...
                                    <td>
                                        <input type="text" class="form-control text-center"
                                            ...
                                            name="packaging_materials[{{ $key+1 }}][category]"
                                            id="packaging_materials_{{ $key+1 }}_category" data-id="{{ $key+1 }}"
                                            readonly>
                                    </td>
                                    <td>
                                        <select class="form-control prefix"
                                            name="packaging_materials[{{ $key+1 }}][prefix]"
                                            id="packaging_materials_{{ $key+1 }}_prefix"
                                            onchange="calculateRowTotal('{{ $key+1 }}')" data-id="{{ $key+1 }}">
                                            <option value="+" {{  $item->pivot->prefix == '+' ? 'selected' : '' }}>+
                                            </option>
                                            <option value="-" {{  $item->pivot->prefix == '-' ? 'selected' : '' }}>-
                                            </option>
                                        </select>
                                    ...
                                    <td>
                                        <input type="hidden" class="form-control" value="{{ $item->unit_id }}"
                                            name="packaging_materials[{{ $key+1 }}][unit_id]"
                                            id="packaging_materials_{{ $key+1 }}_unit_id" data-id="{{ $key+1 }}">
                                        <input type="text" class="form-control text-center"
                                            value="{{ $item->unit->unit_name }}"
                                            name="packaging_materials[{{ $key+1 }}][unit_name]"
                                            id="packaging_materials_{{ $key+1 }}_unit_name" data-id="{{ $key+1 }}"
                                            readonly>
                                    </td>
                                    <td>
                                        <input type="text" class="form-control text-right"
                                            value="{{ $item->pivot->rate }}"
                                            name="packaging_materials[{{ $key+1 }}][rate]"
                                            id="packaging_materials_{{ $key+1 }}_rate" data-id="{{ $key+1 }}" readonly>
                                    </td>
                                    <td>
                                        <input type="text" class="form-control text-right qty"
                                            value="{{ $item->pivot->qty }}"
                                            name="packaging_materials[{{ $key+1 }}][qty]"
                                            id="packaging_materials_{{ $key+1 }}_qty"
                                            onkeyup="calculateRowTotal('{{ $key+1 }}')" data-id="{{ $key+1 }}">
                                    </td>
                                    <td>
                                        <input type="text" class="form-control text-right total"
                                            value="{{ $item->pivot->total }}"
                                            name="packaging_materials[{{ $key+1 }}][total]"
                                            id="packaging_materials_{{ $key+1 }}_total" data-id="{{ $key+1 }}" readonly>
                                    </td>
                                    <td class="text-center">
                                        <button type="button" class="btn btn-danger btn-sm remove_material"> <i
                                ...
                                <tr>
                                    <td>
                                        <select class="form-control selectpicker"
                                            name="packaging_materials[1][material_id]"
                                            id="packaging_materials_1_material_id" data-id="1" onchange="setRowValue(1)"
                                    ...
                                    <td>
                                        <input type="text" class="form-control text-center"
                                            name="packaging_materials[1][category]" id="packaging_materials_1_category"
                                            data-id="1" readonly>
                                    </td>
                                    <td>
                                        <select class="form-control prefix" name="packaging_materials[1][prefix]"
                                            id="packaging_materials_1_prefix" data-id="1">
                                            <option value="+">+</option>
                                            <option value="-">-</option>
                                        </select>
                                    </td>
                                    <td>
                                        <input type="hidden" class="form-control" name="packaging_materials[1][unit_id]"
                                            id="packaging_materials_1_unit_id" data-id="1">
                                        <input type="text" class="form-control text-center"
                                            name="packaging_materials[1][unit_name]"
                                            id="packaging_materials_1_unit_name" data-id="1" readonly>
                                    </td>
                                    <td>
                                        <input type="text" class="form-control text-right"
                                            name="packaging_materials[1][rate]" id="packaging_materials_1_rate"
                                            data-id="1" readonly>
                                    </td>
                                    <td>
                                        <input type="text" class="form-control text-right qty"
                                            name="packaging_materials[1][qty]" id="packaging_materials_1_qty"
                                            onkeyup="calculateRowTotal(1)" data-id="1">
                                    </td>
                                    <td>
                                        <input type="text" class="form-control text-right total"
                                            name="packaging_materials[1][total]" id="packaging_materials_1_total"
                                            data-id="1" readonly>
                                    </td>
                            ...
                            <tfoot>
                                <tr>
                                    <td colspan="7"><button type="button" class="btn btn-primary btn-sm"
                                            id="add_more_material"> <i class="fas fa-plus-square text-white"></i>
                                        </button></td>
                                </tr>
                            </tfoot>
                        </table>
                        ...
                        <table class="table">
                            <tr>
                                ...
                                <td class="w-50"><input type="text" class="form-control text-right font-weight-bolder"
                                        value="{{ number_format($production->total_cost,4,'.','') }}" name="total_cost"
                                        id="total_cost" readonly /></td>
                            </tr>
                            <tr>
                                <td class="text-right font-weight-bolder w-50">Extra Cost</td>
                                <td class="w-50"><input type="text" class="form-control text-right font-weight-bolder"
                                        value="{{ number_format($production->extra_cost,4,'.','')}}"
                                        onkeyup="totalCostCalculation()" name="extra_cost" id="extra_cost" /></td>
                            </tr>
                            <tr>
                                <td class="text-right font-weight-bolder w-50">Total Net Cost</td>
                                <td class="w-50"><input type="text" class="form-control text-right font-weight-bolder"
                                        value="{{ number_format($production->total_net_cost,4,'.','')}}"
                                        name="total_net_cost" id="total_net_cost" readonly /></td>
                            </tr>
                            <tr>
                                <td class="text-right font-weight-bolder w-50">Per Unit Cost</td>
                                <td class="w-50"><input type="text" class="form-control text-right font-weight-bolder"
                                        value="{{ number_format($production->per_unit_cost,4,'.','')}}"
                                        name="per_unit_cost" id="per_unit_cost" readonly /></td>
                            </tr>
                        </table>
                    </div>
                    <div class="col-md-12 text-right pt-5">
                        <button type="button" class="btn btn-primary btn-sm" onclick="check_material_stock()"
                            id="save-btn"><i class="fas fa-save"></i> Update</button>
                    </div>
                </form>
            </div>
...
function update_data(){
    let form = document.getElementById('store_or_update_form');
    let formData = new FormData(form);
    let url = "{{url('pre-production/update')}}";
    $.ajax({
        url: url,
        type: "POST",
        data: formData,
